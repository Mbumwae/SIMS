...
class WLSM_M_Staff_Class
{
	...
	public static function get_class_students($school_id, $session_id, $class_id, $only_active = true)
	{
		...
		$students = $wpdb->get_results($wpdb->prepare('SELECT sr.ID, sr.name, sr.admission_number, sr.roll_number, sr.phone, sr.blood_group, sr.note, se.label as section_label FROM ' . WLSM_STUDENT_RECORDS . ' as sr
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
	...
	public static function get_roll_no_text($roll_number)
	{
		...
		public static function get_id_number_text($id_number)
	{
		if ($id_number) {
			return $id_number;
		}
