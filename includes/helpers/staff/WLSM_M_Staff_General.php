	...
	public static function fetch_students_query($school_id, $session_id, $filter, $restrict_to_section = false)
{
    ...
    $where = '';
    if (isset($filter['search_by']) && $filter['search_by'] === 'search_by_keyword' && !empty($filter['search_field']) && !empty($filter['search_keyword'])) {
        $where .= ' AND sr.' . $filter['search_field'] . ' LIKE "%' . sanitize_text_field($filter['search_keyword']) . '%"';
    }

    $query = 'SELECT sr.ID, sr.name as student_name, sr.phone, sr.student_type, sr.photo_id, sr.email, sr.address, sr.city, sr.title, sr.marital, sr.consti, sr.state, sr.country, sr.gender, sr.dob, sr.religion, sr.caste, sr.blood_group, sr.father_occupation, sr.mother_name, sr.mother_phone, sr.mother_occupation, sr.father_name, sr.father_phone, sr.admission_number, sr.enrollment_number, sr.admission_date, sr.from_front, sr.gdpr_agreed, c.label as class_label, se.label as section_label, sr.roll_number, sr.is_active, u.user_email as login_email, u.user_login as username';
    // Add 'id_number' field to the select statement.
    $query .= ', sr.id_number';
    $query .= ', sr.note';
    $query .= ' FROM ' . WLSM_STUDENT_RECORDS . ' as sr
            JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
            JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
            JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	public static function fetch_student( $school_id, $session_id, $id, $restrict_to_section = false ) {
		...
		$student = $wpdb->get_row( $wpdb->prepare( 'SELECT sr.ID, sr.name as student_name, sr.gender, sr.phone, sr.email, sr.address, sr.city, sr.state, sr. title, sr. marital, sr. consti, sr.country, sr.religion, sr.caste, sr.blood_group, sr.dob, sr.father_name, sr.student_type, sr.father_phone, sr.father_occupation, sr.mother_name, sr.mother_phone, sr.mother_occupation, sr.admission_number, sr.room_id, sr.enrollment_number, sr.admission_date, sr.photo_id, sr.id_number, sr.id_proof, sr.nrc, sr.parent_id_proof, sr.note, sr.route_vehicle_id, sr.parent_user_id, c.ID as class_id, c.label as class_label, se.ID as section_id, se.label as section_label, se.class_school_id, sr.roll_number, sr.is_active, ss.label as session_label, ss.start_date, ss.end_date, u.user_email as login_email, u.user_login as username, tf.from_student_record, tf.to_school FROM ' . WLSM_STUDENT_RECORDS . ' as sr
		JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
		JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
		JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
	...
	public static function get_gender_text( $gender ) {
		...
		if ( isset( WLSM_Helper::term_list()[ $note ] ) ) {
			return WLSM_Helper::term_list()[ $note ];
		}
		return '';
	}

	public static function get_blood_group_text( $blood_group ) {
		if ( isset( WLSM_Helper::sponsorship_list()[ $blood_group ] ) ) {
			return WLSM_Helper::sponsorship_list()[ $blood_group ];
		}
		return '';
	}
