		...
		try {
			...
			if ( count( $classes ) ) {
				foreach ( $classes as $class_id ) {
					...
					if ( ! $class_school ) {
						$errors['classes[]'] = esc_html__( 'Year of Study not found.', 'school-management' );
			...
			if ($current_user) {
				...
				if ($restrict_to_section) {
					...
					if (($restrict_to_section_detail->class_id )!== $class_id) {
						$errors['classes[]'] = esc_html__("You don't have permission to save for this Year of Study.", 'school-management');
					}
				}
			}

			if ( ! count( $class_schools ) ) {
				$errors['classes[]'] = esc_html__( 'Please select at least one Year of Study.', 'school-management' );
			}
			...
			if ( empty( $class_id ) ) {
				...
				if ( ! $class_school ) {
					...
					if ( ! empty( $section_id ) ) {
						...
						$section_label = esc_html__( 'All Programmes', 'school-management' );
					}
				}
			}
		...
		if ( count( $errors ) < 1 ) {
			try {
							...
							<div class="wlsm-form-sub-heading-small wlsm-font-bold">
								...
								printf(
									wp_kses(
										...
										__( 'Exam Results Assessment - Year of Study: <span class="text-secondary">%1$s</span> | Programme: <span class="text-secondary">%2$s</span>', 'school-management' ),
					...
					<div class="col-md-12 text-center">
						...
						printf(
							wp_kses(
								...
								__( 'Academic Year: <span class="wlsm-font-bold">%s</span>', 'school-management' ),
								array( 'span' => array( 'class' => array() ) )
							),
				...
				<div class="alert alert-warning wlsm-font-bold">
					...
					<?php esc_html_e( 'There is no student in this Year of Study or Programme.', 'school-management' ); ?>
				</div>
				<?php
				}
				$html = ob_get_clean();
