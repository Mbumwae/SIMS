			...
			<div class="card-header">
				<span class="float-left h5">
					...
					printf(
						wp_kses(
							...
							__( '<span class="text-secondary">Year of Study:</span> %s', 'school-management' ),
							array(
				...
				<span class="float-md-right">
					<a href="<?php echo esc_url( $page_url ); ?>" class="btn btn-sm btn-info">
						...
						<?php esc_html_e( 'View Years of Study', 'school-management' ); ?>
					</a>
				</span>
			</div>
				...
				<div class="row<?php if ( $section ) { echo ' justify-content-md-center'; } ?>">
					...
					<div class="col-md-7">
						<h2 class="h4 border-bottom pb-2">
							...
							<?php esc_html_e( 'Year of Study & Programmes', 'school-management' ); ?>
						</h2>
						<table class="table table-hover table-bordered" id="wlsm-class-sections-table" data-class-school="<?php echo esc_attr( $class->ID ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'class-sections-' . $class->ID ) ); ?>">
							<thead>
								<tr class="text-white bg-primary">
									<th scope="col"><?php esc_html_e( 'Programme', 'school-management' ); ?></th>
									<th scope="col"><?php esc_html_e( 'Total Students', 'school-management' ); ?></th>
									<th scope="col"><?php esc_html_e( 'Course Director', 'school-management' ); ?></th>
					...
					<div class="col-md-5">
							...
							<h2 class="h4 border-bottom pb-2 wlsm-page-heading">
								...
								printf(
									wp_kses(
										...
										__( 'Edit Programme: <span class="text-secondary">%s</span>', 'school-management' ),
										array(
											'span' => array( 'class' => array() )
										)
									),
								...
								<a href="<?php echo esc_url( $page_url . '&action=sections&id=' . $class->class_id ); ?>" class="float-md-right btn btn-sm btn-outline-primary">
									...
									<?php esc_html_e( 'Add New Programme', 'school-management' ); ?>
								</a>
								<?php } else { ?>
						...
						<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wlsm-save-section-form">
							...
							<div class="form-group">
								<label for="wlsm_section_label" class="font-weight-bold"><?php esc_html_e( 'Programme', 'school-management' ); ?>:</label>
								<input type="text" name="label" class="form-control" id="wlsm_section_label" placeholder="<?php esc_attr_e( 'Enter Programme', 'school-management' ); ?>" value="<?php echo esc_attr( WLSM_M_Staff_Class::get_section_label_text( $label ) ); ?>">
							</div>

							<?php if ( $section && ( $section_id == $class->default_section_id ) ) { ?>
							<div class="text-secondary mb-1">
								<?php esc_html_e( 'Currently set to default Programme.', 'school-management' ); ?>
							</div>
							<?php } else { ?>
							<div class="form-group">
								...
								<label class="ml-4 mb-1 form-check-label wlsm-font-bold text-dark" for="wlsm_is_default">
									<?php esc_html_e( 'Set as default Programme?', 'school-management' ); ?>
								</label>
							</div>
							<?php } ?>
								...
								<span class="float-md-right">
									<button type="submit" class="btn btn-sm btn-primary" id="wlsm-save-section-btn">
										...
										if ( $section ) {
											...
											<?php
											esc_html_e( 'Update Programme', 'school-management' );
										} else {
											?>
											<i class="fas fa-plus-square"></i>&nbsp;
											<?php
											esc_html_e( 'Add New Programme', 'school-management' );
										}
										?>
									</button>
								</span>
