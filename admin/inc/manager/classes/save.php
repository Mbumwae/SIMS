			...
			<div class="col-md-12">
				<div class="wlsm-main-header card col wlsm-page-heading-box">
					...
					<?php if ( $class ) { ?>
						...
						printf(
							wp_kses(
								...
								__( 'Edit Year of Study: <span class="text-secondary">%s</span>', 'school-management' ),
								array(
					...
					<?php } else { ?>
						...
						<?php esc_html_e( 'Add New Year of Study', 'school-management' ); ?>
					<?php } ?>
					</h1>
				</div>
			</div>
					...
					<div class="card-body">
						<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wlsm-save-class-form">
							...
							<div class="form-group">
								<label for="wlsm_label" class="font-weight-bold"><span class="wlsm-important">*</span> <?php esc_html_e( 'Year of Study', 'school-management' ); ?>:</label>
								<input type="text" name="label" class="form-control" id="wlsm_label" placeholder="<?php esc_attr_e( 'Enter Year of Study', 'school-management' ); ?>" value="<?php echo esc_attr( WLSM_M_Class::get_label_text( $label ) ); ?>">
							</div>
								...
								<span class="float-md-right">
									<button type="submit" class="btn btn-sm btn-primary" id="wlsm-save-class-btn">
										...
										if ( $class ) {
											...
											<?php
											esc_html_e( 'Update Year of Study', 'school-management' );
										} else {
											?>
											<i class="fas fa-plus-square"></i>&nbsp;
											<?php
											esc_html_e( 'Add New Year of Study', 'school-management' );
										}
										?>
									</button>
								</span>
