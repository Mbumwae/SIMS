...
class WLSM_Session {
	...
	public static function save() {
		...
		try {
			...
			if ( $session_id ) {
				...
				if ( ! $session ) {
					throw new Exception( esc_html__( 'Academic Year not found.', 'school-management' ) );
				}
			}
			...
			if ( empty( $label ) ) {
				$errors['label'] = esc_html__( 'Please provide Academic Year.', 'school-management' );
			} else {
				if ( strlen( $label ) > 191 ) {
			...
			if ( $start_date >= $end_date ) {
				$errors['start_date'] = esc_html__( 'Academic Year start date must be lower than end date.', 'school-management' );
			}

			if ( empty( $start_date ) ) {
				$errors['start_date'] = esc_html__( 'Please provide start date of Academic Year.', 'school-management' );
			} else {
				$start_date = $start_date->format( 'Y-m-d' );
			}

			if ( empty( $end_date ) ) {
				$errors['end_date'] = esc_html__( 'Please provide end date of Academic Year.', 'school-management' );
			} else {
			...
			if ( $session_exist ) {
				$errors['label'] = esc_html__( 'Academic Year already exists with this label, start date and end date.', 'school-management' );
			}
			// End validation.
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				if ( $session_id ) {
					...
					$message = esc_html__( 'Academic Year updated successfully.', 'school-management' );
					$reset   = false;
				} else {
					$data['created_at'] = current_time( 'Y-m-d H:i:s' );

					$success = $wpdb->insert( WLSM_SESSIONS, $data );
					$message = esc_html__( 'Academic Year added successfully.', 'school-management' );
					$reset   = true;
				}
	...
	public static function delete() {
		...
		try {
			...
			$message = esc_html__( 'Academic Year deleted successfully.', 'school-management' );
