...
class WLSM_School {
	public static function fetch_schools() {
		...
		if ( count( $filter_rows_limit ) ) {
			foreach ( $filter_rows_limit as $row ) {
				...
				if ( ! $classes_count ) {
					$classes_count = '<a class="text-primary wlsm-font-bold" href="' . esc_url( $page_url . "&action=classes&id=" . $row->ID ) . '">' . esc_html__( 'Assign Year of Study', 'school-management' ) . '</a>';
				} else {
	...
	public static function delete_school_class() {
		...
		try {
			...
			if ( ! $class_school ) {
				throw new Exception( esc_html__( 'Year of Study not found in the school.', 'school-management' ) );
			}
		...
		try {
			...
			$message = esc_html__( 'Year of Study removed from the school successfully.', 'school-management' );
	...
	public static function assign_classes() {
		...
		try {
			...
			if ( ! count( $classes ) ) {
				$errors['keyword'] = esc_html__( 'Please select atleast one Year of Study to assign.', 'school-management' );
			}
			// End validation.
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				$message = esc_html__( 'Year of Study assigned successfully.', 'school-management' );
