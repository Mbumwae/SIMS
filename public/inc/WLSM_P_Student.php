...
class WLSM_P_Student {
	...
	public static function view_homework() {
		...
		try {
			...
			if ( ! $homework ) {
				throw new Exception( esc_html__( 'Task not found.', 'school-management' ) );
			}
    ...
    try {
        ...
        if (isset($attachment['tmp_name']) && !empty($attachment['tmp_name'])) {
            if (!WLSM_Helper::is_valid_file($attachment, 'application/pdf')) {
                $errors['attachment'] = esc_html__('Please provide attachment in PDF format.', 'school-management');
            }
        }
    ...
    if (count($errors) < 1) {
        try {
            ...
            if (!empty($attachment)) {
                $attachment = media_handle_upload('attachments', 0, array('mimes' => 'application/pdf'));
